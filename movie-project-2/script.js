...
document.getElementById('search').onsubmit = async function (event) {
  ...
  for(var item of document.querySelectorAll('.single-movie-btn')) {
    item.onclick = async function (event) {
      
      const res = await fetch(`http://www.omdbapi.com/?i=${event.target.parentElement.dataset.imdbid}&apiKey=9606ae0f`)
